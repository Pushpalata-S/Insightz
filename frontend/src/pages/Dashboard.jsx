...
export default function Dashboard() {
  ...
  // --- OPTION 1: QUICK SUMMARY (Metadata) ---
  const handleQuickSummary = (e, doc) => {
    e.stopPropagation(); 
    setActiveTab('search');
    const safeSummary = doc.summary || "No preview available.";
    setSearchResult(`### ⚡ Quick Preview: ${doc.filename}\n\n${safeSummary}\n\n*(Stored metadata only)*`);
  };

  // --- OPTION 2: DEEP SUMMARY (Live AI Generation) ---
  const handleDeepSummary = async (e, doc) => {
    e.stopPropagation(); 
    setActiveTab('search');
    setSearchResult(`🧠 Generating Deep Analysis for "${doc.filename}"... \n\n*Reading full document content...*`);

    try {
      // Re-use cross-summary endpoint for single file deep dive
      const res = await api.post('/cross-summary', { 
        filenames: [doc.filename] 
      });
      setSearchResult(`### 🧠 Deep Analysis: ${doc.filename}\n\n${res.data.cross_summary}`);
    } catch (err) {
      setSearchResult(`❌ Error generating deep summary. \n\nServer response: ${err.message}`);
    }
  };

  // --- PAGE WISE SUMMARY NAV ---
          ...
          {docs.map(doc => (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '5px' }}>
                  <button 
                    onClick={(e) => handleQuickSummary(e, doc)} 
                    style={{ background: '#2c3e50', border: 'none', color: '#f1c40f', borderRadius: '4px', padding:'4px 8px', cursor: 'pointer', fontSize:'0.8em' }}
                    title="Quick Summary (Preview)"
                  >
                    ⚡
                  </button>
                  <button 
                    onClick={(e) => handleDeepSummary(e, doc)} 
                    style={{ background: '#2c3e50', border: 'none', color: '#3498db', borderRadius: '4px', padding:'4px 8px', cursor: 'pointer', fontSize:'0.8em' }}
        ...
        {activeTab === 'search' && (
          ...
          >
            ...
            <div 
                ...
                dangerouslySetInnerHTML={{ __html: searchResult === 'Ready for query.' || searchResult === 'Searching...' || searchResult.startsWith('🧠') ? searchResult : marked.parse(searchResult) }}
            />
          </div>
        )}
